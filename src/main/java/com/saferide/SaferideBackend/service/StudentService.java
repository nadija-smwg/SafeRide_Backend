...
public class StudentService { // for all student database operations
    ...
    public String saveStudent(Student student) {
        ...
        if (student.getStudentId() == null || student.getStudentId().isEmpty()) {
            student.setStudentId("STU-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase()); // UUID -
                                                                                                       // a1b2c3d4e5f6,Final
                                                                                                       // - STU-A1B2C3D4
        }

        Firestore db = FirestoreClient.getFirestore(); // connect to Firebase
        String studentId = java.util.Objects.requireNonNull(student.getStudentId(), "Student ID must not be null"); // ensure
                                                                                                                    // ID
                                                                                                                    // not
                                                                                                                    // null,safety
                                                                                                                    // check
    ...
    public Student getStudentById(String studentId) {
        ...
        ApiFuture<DocumentSnapshot> future = docRef.get(); // request to fetch doc/data, Firestore does not return data
                                                           // immediately
        try {
    ...
    public void updateStudentStatus(String studentId, String status) {
        Firestore db = FirestoreClient.getFirestore();
        try {
            // Update currentStatus field in Firestore(called by attendanceservice and
            // change the status.)
            db.collection(COLLECTION_NAME).document(java.util.Objects.requireNonNull(studentId))
                    .update("currentStatus", status).get();
        } catch (InterruptedException | ExecutionException e) {
