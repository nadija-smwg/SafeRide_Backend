...
public class ScanController {
    ...
    // PathVariable- Extract student ID from URL and Optional query params for trip
    // and location
    public ResponseEntity<?> scanPickup(@PathVariable String studentId, @RequestParam String tripId,
        ...
        try {
            ...
            AttendanceRecord record = new AttendanceRecord(UUID.randomUUID().toString(), studentId, "PICKED_UP", tripId,
                    lat, lon, false);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error during pickup: " + e.getMessage());
        }
    }

    @PostMapping("/dropoff/{studentId}")
    public ResponseEntity<?> scanDropoff(@PathVariable String studentId, @RequestParam String tripId,
            @RequestParam(required = false) Double lat, @RequestParam(required = false) Double lon) {
        try {
            Student student = studentService.getStudentById(studentId);
            if (student == null) {
            ...
            AttendanceRecord record = new AttendanceRecord(UUID.randomUUID().toString(), studentId, "DROPPED_OFF",
                    tripId, lat, lon, false);
            attendanceService.saveRecord(record);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error during dropoff: " + e.getMessage());
        }
    }

    @PostMapping("/manual")
    public ResponseEntity<?> scanManual(@RequestParam String studentId, @RequestParam String status,
            @RequestParam String tripId, @RequestParam(required = false) Double lat,
        ...
        try {
            ...
            AttendanceRecord record = new AttendanceRecord(UUID.randomUUID().toString(), studentId, status, tripId, lat,
                    lon, true);
            attendanceService.saveRecord(record);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error during manual attendance: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> generateQR(@PathVariable String studentId) {
        // Returns the Base64 image which frontend can use like: <img
